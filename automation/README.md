...
4. 客訴趨勢洞察（MoM／QoQ）——月度／季度趨勢、KPI總覽、結構變化重點（詳見下方「第四頁」章節）
...
| 檔案 | 用途 | 更新頻率 |
|---|---|---|
| `客訴內容分析.csv` | 全部客訴原始紀錄，程式自動抓「上一個完整週」；同一份資料也用於第四頁 MoM/QoQ 動態彙總 | 建議每週一早上、排程執行前更新 |
...
新增第四頁投影片，聚焦月度（MoM）與季度（QoQ）趨勢分析：

- **重點結論橫幅**：一句話彙整本月／本季客訴量方向與本季結構變化最大項，供主管快速掌握
- **KPI 卡片（4張）**：本月客訴總量(MoM)、客訴密度指標(每站平均客訴量，暫代萬人客訴率)、本季客訴總量(QoQ)、機台問題佔比(QoQ)
- **圖表A**：總客訴量與客訴密度雙軸趨勢圖（近6個完整月，動態抓取）
- **圖表B**：客訴主題結構比變化（QoQ，100%堆疊橫條圖，含百分比座標軸與資料標籤）
- **本季結構變化重點**：動態找出QoQ佔比變化最大的問題類型並如實描述（不假設季節性因果，待累積滿一年以上同季資料才會納入季節性判斷）

### 資料需求與限制
- MoM/QoQ 計算直接由 `客訴內容分析.csv` 的歷史列動態彙總（該檔案本身即保留全年歷史，不需額外累積檔）
- 萬人客訴率因缺乏「站點覆蓋用戶數」資料源，暫以「每站平均客訴量」替代，介面已預留未來直接替換公式
- 新增 `monthly_low_volume_history.json`：累積每月低回收量Top10名單，供未來「高風險站點長效追蹤」功能使用（目前未顯示於投影片）
- 若要啟用「系統／活動影響評估」洞察，需在 `config.json` 新增 `activity_calendar_path` 指向活動時間表 CSV（欄位：日期、活動名稱），未設定時該段落自動略過，不影響其餘頁面產出
